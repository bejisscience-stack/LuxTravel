...
export async function POST() {
  ...
  try {
    ...
    const existingResult = await supabase
      .from('buses')
      .select('id, name')
    const existingBuses = existingResult.data as { id: string; name: string }[] | null
    ...
    for (const busData of SEED_BUSES) {
      ...
      if (existing) {
        ...
        const { error } = await (supabase
          .from('buses') as unknown as { update: (data: Record<string, unknown>) => { eq: (col: string, val: string) => Promise<{ error: { message: string } | null }> } })
          .update({
            ...busData,
        ...
        // Insert new bus
        const { error } = await (supabase
          .from('buses') as unknown as { insert: (data: Record<string, unknown>[]) => Promise<{ error: { message: string } | null }> })
          .insert([{
            ...busData,
            amenities: JSON.stringify(busData.amenities),
          }])
